            ...
            if check["permissions"] == "wl" or ctx.author.id in self.bot.config["owners"]:
                ...
                try:
                    ...
                    if i.ctx.custom_id == "claim":
                        ...
                        self.bot.bdd.query(f"UPDATE profiles SET coins=coins+{coins},points=points+{points},tokens=tokens+{tokens},rob_availables=rob_availables+{pillages} WHERE id={i.ctx.author.id}")
