...
class PanelEventRoulette(Extension):
    ...
    async def panel_roulette_callback(self,ctx):
        ...
        if u == []:
            await ctx.send("Vous n'avez pas de compte ! Créez en un en appuyant sur le bouton 'profil' !",ephemeral=True)
            return
        u = u[0]
        categories = bdd.get_roulette_category()
        desc = ""
        if categories == []:
            await ctx.send("Aucune catégorie de roulette n'a été configurée, veuillez contacter un administrateur.",ephemeral=True)
            return
    ...
    async def tirage_1_callback(self,ctx):
        ...
        u = u[0]
        if u["tokens"] < 1:
            await ctx.send(f"Vous n'avez pas de jeton disponible. Allez en vocal, parlez dans le chat ou mettez {c['status']} en statut pour en gagner",ephemeral=True)
            return
        items = bdd.get_roulette_items()
        if items == []:
            await ctx.send("Aucun item n'a été configuré, veuillez contacter un administrateur.",ephemeral=True)
            return
        rarity = [rar["rarity"] for rar in items]
        if ctx.author.id in opening:
            await ctx.send("Vous avez déjà un tirage en cours, veuillez attendre la fin de celui-ci.",ephemeral=True)
            return
        ...
        if chosend_item["type"] == "coins":
            ...
            await ctx.send(f"Vous avez gagné {chosend_item['name']} ({chosend_item['data']} coins) !",ephemeral=True)
        ...
        if chosend_item["type"] == "tokens":
            ...
            await ctx.send(f"Vous avez gagné {chosend_item['name']} ({chosend_item['data']} jetons) !",ephemeral=True)
        ...
        if chosend_item["type"] == "role":
            ...
            if r in ctx.author.roles:
                ...
                await ctx.send(f"Vous avez déjà le role {chosend_item['name']} (<@&{chosend_item['data']}>) ! \n Vous avez reçu un dédomagement de {r} coins !",ephemeral=True)
                return
            try:
        ...
        if chosend_item["type"] == "badge":
            ...
            if str(chosend_item["data"]) in badges:
                ...
                await ctx.send(f"Vous avez déjà le badge {chosend_item['name']} (<@&{chosend_item['data']}>) ! \n Vous avez reçu un dédomagement de {r} coins !",ephemeral=True)
                return
    ...
    async def tirage_5_callback(self,ctx):
        ...
        u = u[0]
        if u["tokens"] < 5:
            await ctx.send(f"Vous n'avez pas 5 jetons disponibles. Allez en vocal, parlez dans le chat ou mettez {c['status']} en statut pour en gagner",ephemeral=True)
            return
        items = bdd.get_roulette_items()
        if items == []:
            await ctx.send("Aucun item n'a été configuré, veuillez contacter un administrateur.",ephemeral=True)
    ...
    async def tirage_10_callback(self,ctx):
        ...
        u = u[0]
        if u["tokens"] < 10:
            await ctx.send(f"Vous n'avez pas 10 jetons disponibles. Allez en vocal, parlez dans le chat ou mettez {c['status']} en statut pour en gagner",ephemeral=True)
            return
        items = bdd.get_roulette_items()
        if items == []:
            await ctx.send("Aucun item n'a été configuré, veuillez contacter un administrateur.",ephemeral=True)
